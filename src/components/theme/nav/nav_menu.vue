  ...
  <section class="m_nav_menu">
    ...
    >
      <li 
        ...
        :key="index" 
        :class="[item.path === '/home' ? 'm_undraggable' : 'm_darg_btn', item.path === routerConfig.path ? 'm_hight_list' : '']" @click="jump(item)"
      >
        {{item.title}}
        <i v-if="item.path !== '/home'" @click.stop="removeNav(index)" class="el-icon-close m_drag_list_remove"></i>
      </li>
    </draggable>
  </section>
</template>
